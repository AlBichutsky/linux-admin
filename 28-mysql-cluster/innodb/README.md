...
- ```mysql-router``` - обеспечивает маршрутизацию соединений с серверами MySQL в кластере для повышения производительности и надежности,
при его настройке указывается первичная нода (Primary instance) для того, чтобы роутер узнал топологию кластера
...
В каталоге /data хранятся папки, которые монтируются в контейнерах и служат для хранения данных mysql (БД, логи и т.д.).

В docker-compose.yml монтирование выглядит так:
```
 volumes:
      - ./data/node1:/var/lib/mysql
```

mysql-shell и ноды общаются между собой по порту tcp `33060`
...
```
В случае ошибок пересоздаем контейнеры с очисткой данных в папках /data/../ , созданных mysql.
```bash
sudo su
...
Все запросы должны выполняться к ```mysql-router```. Он будет автоматически направлять их на первичную ноду, остальные вторичные ноды - реплика.
Если первичная нода стала недоступной, то в процессе перевыборов назначается новая первичная нода.

Заливаем дамп базы ```bet```:
```bash
...
v_same_event
```
Проверяем откзоустойчивость кластера, отключив первичную ноду ```node1```.
```
docker-compose stop node1
```
Узнаем ip-адрес mysql-router (docker выдает динамические)
```bash
