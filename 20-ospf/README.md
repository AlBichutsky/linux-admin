...
Тестовый стенд разворачивается помощью Vagrant + Ansible
...
5: eth3: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc pfifo_fast state UP group default qlen 1000
    inet 192.168.102.1/24 brd 192.168.102.255 scope global noprefixroute eth3
       valid_lft forever preferred_lft forever
```
#### Команды для просмотра параметров OSPF

Команда для перехода в консоль QUAGGA
```
vtysh
```
Посмотреть БД OSPF
...
```
Как видно, пакеты от abr1 к abr3 уходят уже не через eth2, а через eth1 (через сервер abr2). 
...
[root@abr3 vagrant]# tracepath 192.168.100.1
 ...
 1:  192.168.100.1                                         0.837ms reached
     Resume: pmtu 1500 hops 1 back 1
``` 
Здесь пакеты от abr3 к abr1 уходят через eth1 по обычному кратчайшему маршруту.

Таким образом, роутинг ассиметричен - пакеты в обе стороны идут не симметрично.
...
```
Смотрим маршрут от abr1 к abr3 (он как и ранее идет через abr2):
```
[root@abr1 vagrant]# tracepath 192.168.102.1
 1?: [LOCALHOST]                                         pmtu 1500
...
root@abr3 vagrant]# tracepath 192.168.100.1
 ...
 2:  192.168.100.1                                         0.744ms reached
     Resume: pmtu 1500 hops 2 back 2
```
Как видно, пакеты от abr3 к abr1 уходят уже не через eth1, а через eth2 (через сервер abr2).

Соответственно, роутинг симметричный - пакеты в обе стороны идут симметрично.