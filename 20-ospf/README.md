...
[root@abr3 vagrant]# tracepath 192.168.100.1
 ...
 1:  192.168.100.1                                         0.837ms reached
     Resume: pmtu 1500 hops 1 back 1
``` 
Здесь пакеты с abr3 идут через eth1 по старому короткому маршруту.

Таким образом, роутинг ассиметричный - пакеты от abr1 к abr3 и abr3 к abr1 идут разными машрутами.
...
root@abr3 vagrant]# tracepath 192.168.100.1
 ...
 2:  192.168.100.1                                         0.744ms reached
     Resume: pmtu 1500 hops 2 back 2
```
Как видно, пакеты от abr3 идут уже не через eth1, а через eth2 и проходят по новому маршруту (через сервер abr2 в 2 хопа).

Таким образом, роутинг симметричный - пакеты от abr1 к abr3 и abr3 к abr1 идут по одинаковому маршруту.